...
def compute_log_probs(model_logits,target_labels,assistant_answer_mask):
    ...
    # average_log_probs : [batch_size]
    average_log_probs = masked_answer_probs.sum(-1) / assistant_answer_mask.sum(-1)
...
def compute_loss(chosen_log_probs,rejected_log_probs,reference_chosen_log_probs,reference_rejected_log_probs,beta):
    """
    chosen_log_probs：训练模型输出chosen回答的对数概率, shape:[batch_size]
    rejected_log_probs：训练模型输出rejected回答的对数概率, shape:[batch_size]
    reference_chosen_log_probs：参考模型输出chosen回答的对数概率, shape: [batch_size]
    reference_rejected_log_probs：参考模型输出rejected回答的对数概率, shape:[batch_size]
    """
    train_margin=chosen_log_probs - rejected_log_probs
    ref_margin = reference_chosen_log_probs - reference_rejected_log_probs

    final_margin = train_margin - ref_margin

    # loss.shape:[batch_size,]
    # [0.1, 0.2, 0.3]
    loss = -torch.nn.functional.logsigmoid(beta * final_margin)

    average_batch_loss = loss.sum(-1) / len(loss)
